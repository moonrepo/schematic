...
impl TypeScriptGenerator {
    ...
    pub fn add<T: Config>(&mut self) -> miette::Result<&mut Self> {
        let mut fields = vec![];
    ...
    pub fn generate(self) -> miette::Result<()> {
        self.generate_with_options(RenderOptions::default())
    }

    pub fn generate_with_options(self, options: RenderOptions) -> miette::Result<()> {
        let mut outputs = vec![
        ...
        for output in &self.outputs {
            outputs.push(render_output(output, false, &options).into_diagnostic()?);

            if !options.exclude_partial && matches!(output, Output::Struct { .. }) {
                outputs.push(render_output(output, true, &options).into_diagnostic()?);
            }
        }
