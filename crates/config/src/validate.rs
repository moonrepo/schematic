...
pub type Validator<T, D> = Box<dyn FnOnce(&T, &D) -> Result<(), ValidateError>>;
...
pub fn alphanumeric<T: Alphanumeric, D>(value: &T, _: &D) -> Result<(), ValidateError> {
...
pub fn ascii<T: Ascii, D>(value: &T, _: &D) -> Result<(), ValidateError> {
...
pub fn contains<T: Contains, D>(pattern: &str) -> Validator<T, D> {
    let pattern = pattern.to_owned();

    Box::new(move |value, _| r::contains::apply(value, (&pattern,)).map_err(map_err))
}
...
pub fn email<T: Email, D>(value: &T, _: &D) -> Result<(), ValidateError> {
...
pub fn regex<T: Pattern, D>(pattern: &str) -> Validator<T, D> {
    let pattern = r::pattern::Regex::new(pattern).unwrap();

    Box::new(move |value, _| r::pattern::apply(value, (&pattern,)).map_err(map_err))
}

pub use r::length::{HasLength, Length};

/// Validate a value is at least the provided length.
pub fn min_length<T: Length, D>(min: usize) -> Validator<T, D> {
    Box::new(move |value, _| r::length::apply(value, (min, usize::MAX)).map_err(map_err))
}

/// Validate a value is at most the provided length.
pub fn max_length<T: Length, D>(max: usize) -> Validator<T, D> {
    Box::new(move |value, _| r::length::apply(value, (usize::MIN, max)).map_err(map_err))
}

/// Validate a value is within the provided length.
pub fn in_length<T: Length, D>(min: usize, max: usize) -> Validator<T, D> {
    Box::new(move |value, _| r::length::apply(value, (min, max)).map_err(map_err))
}

/// Validate the value is not empty.
pub fn not_empty<T: HasLength, D>(value: &T, _: &D) -> Result<(), ValidateError> {
    if value.length() == 0 {
...
pub fn url<T: Url, D>(value: &T, _: &D) -> Result<(), ValidateError> {
    r::url::apply(value, ()).map_err(map_err)
}

#[cfg(feature = "valid_url")]
pub fn url_secure<T: AsRef<str>, D>(value: T, data: &D) -> Result<(), ValidateError> {
    url(&value, data)?;
...
pub fn in_range<T: Bounds + 'static, D>(min: T, max: T) -> Validator<T, D> {
    Box::new(move |value, _| r::range::apply(value, (&min, &max)).map_err(map_err))
}

/// Validate an extends value is either a file path or secure URL.
#[allow(unused_variables)]
pub fn extends_string<D>(value: &str, data: &D) -> Result<(), ValidateError> {
    #[allow(unreachable_code)]
    if is_url_like(value) {
        #[cfg(feature = "valid_url")]
        {
            return url_secure(value, data);
        }
...
pub fn extends_list<D>(values: &[String], data: &D) -> Result<(), ValidateError> {
    for (i, value) in values.iter().enumerate() {
        if let Err(mut error) = extends_string(value, data) {
...
pub fn extends_from<D>(value: &ExtendsFrom, data: &D) -> Result<(), ValidateError> {
    match value {
        ExtendsFrom::String(string) => extends_string(string, data)?,
        ExtendsFrom::List(list) => extends_list(list, data)?,
    };
